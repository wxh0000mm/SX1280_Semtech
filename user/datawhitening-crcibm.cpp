...
//CRC16 校验程序 
// ---------------- POPULAR POLYNOMIALS ---------------- 
//CRC-16:   G(x)=x^16 + x^15 + x^2 + x^0                  (0x8005) 
 
#define                  CRC_16_POLYNOMIALS            0x8005 
 
// -------------------------------------------------------------- 
//            CRC16计算方法:使用2 个256 长度的校验表 
// -------------------------------------------------------------- 
const unsigned int chCRC16_HTalbe[256]=               // CRC  高位字节值表          
{ 
...
const unsigned int chCRC16_LTalbe[256]=                       // CRC  低位字节值表                                          
{ 
...
#define DATA_WHITENING_KEY  0x1FF

// Whitening_Key 获取函数，初始值0x1ff;
void vWht_GetNewKey( unsigned char *pbWhtKey_CarryPtr, unsigned char *pbWhtKey_LowBytePtr )	// generate a new whitening key (carry bit + key)
{
	unsigned char bCnt;
...
//白化函数
void vWht_DataWhitening( unsigned char *pbData, unsigned char length )		 // encrypt/decrypt input data by the whitening key
{
	unsigned char bCnt;
...
unsigned int myCRC16_Check (unsigned char* pchMsg, unsigned int wDataLen)   
{ 
  unsigned char chCRCHi = 0xFF;    //  高CRC字节初始化 
  unsigned char chCRCLo = 0xFF;    //  低CRC字节初始化 
  unsigned int wIndex;              // CRC 循环中的索引 
  while (wDataLen--) 
  { 
    //  计算CRC 
    wIndex = chCRCLo ^ *pchMsg++ ; 
    chCRCLo = chCRCHi ^ chCRC16_HTalbe[wIndex];   
    chCRCHi = chCRC16_LTalbe[wIndex] ; 
  }   
